...
int count_primes(int n) {
    // This may be space-inefficient on some platforms. Normally bool takes one byte, but
    // std::vector<bool> should be optimized. 'std::vector<bool> is a possibly space-efficient
    // specialization' - notice the word 'possibly', the memory optimization is implementation
    // defined, but should be there on most common platforms (I tested it on Ubuntu on x86-64
    // platform, it is optimized). An alternative would be std::bitset (but the issue is that size
    // must be known at compilation time) or simply a custom container.
